...
void line_analize(std::string line)
{
    ...
    std::regex ifRex("if");
    std::regex elseRex("else");
    std::regex whileRex("while");
    std::regex continueRex("continue");
    std::regex breakRex("break");
    ...
    std::regex tokenRegex(R"((\b(int|main|void|return|if|else|while|continue|break)\b|\|\||&&|==|!=|<=|>=|<<|>>|[-+*/%&|^~<>!]|=|[a-zA-Z_][a-zA-Z0-9_]*|[(){}[\]]|,|;|[0-9]+))");
    ...
    for (std::sregex_iterator i = words_begin; i != words_end; ++i)
    {
        ...
        else if (std::regex_match(match_str, ifRex))
        {
            tword.wtype = WordType::IF;
            tword.name = match_str;
            tokens1.push_back(tword);
            line_tokens.push_back(tword);
        }
        else if (std::regex_match(match_str, elseRex))
        {
            tword.wtype = WordType::ELSE;
            tword.name = match_str;
            tokens1.push_back(tword);
            line_tokens.push_back(tword);
        }
        else if (std::regex_match(match_str, whileRex))
        {
            tword.wtype = WordType::WHILE;
            tword.name = match_str;
            tokens1.push_back(tword);
            line_tokens.push_back(tword);
        }
        else if (std::regex_match(match_str, continueRex)){
            tword.wtype = WordType::CONTINUE;
            tword.name = match_str;
            tokens1.push_back(tword);
            line_tokens.push_back(tword);
        }
        else if (std::regex_match(match_str, breakRex)){
            tword.wtype = WordType::BREAK;
            tword.name = match_str;
            tokens1.push_back(tword);
            line_tokens.push_back(tword);
        }
